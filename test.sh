#!/bin/bash
# test.sh — E-Commerce Integration Tests (Robust Version)
...
check "POST /products valide → 201" "201" "$(http -X POST $CAT/products -H 'Content-Type: application/json' -d '{"name":"Produit Test","price":29.99,"stock":5,"category":"other"}')"
check "POST /products prix négatif → 400" "400" "$(http -X POST $CAT/products -H 'Content-Type: application/json' -d '{"name":"Bad","price":-10,"stock":5,"category":"other"}')"
check "POST /products catégorie invalide → 400" "400" "$(http -X POST $CAT/products -H 'Content-Type: application/json' -d '{"name":"Bad","price":10,"stock":5,"category":"INVALIDE"}')"
check "POST /products name vide → 400" "400" "$(http -X POST $CAT/products -H 'Content-Type: application/json' -d '{"name":"","price":10,"stock":5,"category":"other"}')"

PROD_RESP="$(body -X POST $CAT/products -H 'Content-Type: application/json' -d '{"name":"Produit Perishable","price":5.00,"stock":3,"category":"food"}')"
check_contains "POST /products → réponse contient id" '"id"' "$PROD_RESP"

# ── 4. Panier ─────────────────────────────────────────────────
echo -e "
--- 4. Panier ---"
USER="test-user-$RANDOM"
check "GET /cart/:userId crée panier vide" "200" "$(http $BASE/cart/$USER)"

check "POST /cart/:userId/items → 201" "201" "$(http -X POST $PAN/cart/$USER/items -H 'Content-Type: application/json' -d '{"productId":2,"quantity":3,"unitPrice":89.99,"productName":"Clavier"}')"
...
check "POST même productId → quantité incrémentée" "201" "$(http -X POST $PAN/cart/$USER/items -H 'Content-Type: application/json' -d '{"productId":2,"quantity":1,"unitPrice":89.99,"productName":"Clavier"}')"

CART2="$(body $PAN/cart/$USER)"
check_contains "Même produit → quantité incrémentée (itemCount=4)" '"itemCount":4' "$CART2"

check "POST quantity invalide → 400" "400" "$(http -X POST $PAN/cart/$USER/items -H 'Content-Type: application/json' -d '{"productId":1,"quantity":-1,"unitPrice":10}')"
...
ORDER_RESP="$(body -X POST $CMD/orders -H 'Content-Type: application/json' -d '{"userId":"'$USER'","items":[{"productId":2,"productName":"Clavier","quantity":1,"unitPrice":89.99}],"shippingAddress":"1 rue Test, 75001 Paris"}')"
...
check "PATCH status pending→confirmed" "200" "$(http -X PATCH $CMD/orders/$ORDER_ID/status -H 'Content-Type: application/json' -d '{"status":"confirmed"}')"
check "PATCH status invalide → 409" "409" "$(http -X PATCH $CMD/orders/$ORDER_ID/status -H 'Content-Type: application/json' -d '{"status":"pending"}')"
...
check "POST /notify type invalide → 400" "400" "$(http -X POST $NOT/notify -H 'Content-Type: application/json' -d '{"type":"type_qui_nexiste_pas","userId":"test"}')"
